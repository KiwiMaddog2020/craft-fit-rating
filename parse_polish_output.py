...
Score-extraction order (first hit wins):
    1. `<Plan|Code> quality: craft X / fit Y.` aggregate line (the aggregate line the report template emits)
...
Verdict extraction prefers lines starting with `Verdict:`.
...
# Aggregate score line — required by the report template (the aggregate line).
...
# Legacy prose patterns, kept for backward compatibility with older reports.
...
# Verdict — prefer a labeled line (starts with "Verdict:")
...
# Canonical scalars (the aggregate accounting line).
...
#   - SEVERITY | LOCATION | SUMMARY | INTRO_PASS | AUTHOR
OPEN_FINDING_LINE_RE = re.compile(
    r"^\s*-\s*(CRITICAL|HIGH|MEDIUM|LOW)\s*\|\s*([^|]+?)\s*\|\s*([^|]+?)\s*\|\s*(\d+)\s*\|\s*([\w-]+)\s*$",
    re.IGNORECASE | re.MULTILINE,
)

# Legacy severity counter (diagnostic only; the canonical scalar is preferred).
...
def _extract_section(text: str, header_pattern: str) -> str:
    ...
    Section scoping: canonical-scalar and open-findings
